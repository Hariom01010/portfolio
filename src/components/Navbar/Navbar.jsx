...
function Navbar() {
  ...
  return (
    <div className='px-2 py-6 flex justify-between items-center'>
        ...
        {/* Mobile Navbar */}
        <div className={`${navbar?"hidden":"block"} md:hidden`} onClick={showNavbar} ><GiHamburgerMenu size={25} /></div>
        <div className={`${navbar?"block":"hidden"} right-0 top-0 w-2/4 h-full fixed  bg-[#1c1c21] px-4 border-l animate-slideInRight shadow-[5px_0px_10px_10px_rgb(256,256,256)]`}>
            <div><RxCross2 size={28} className='mt-6' onClick={showNavbar}/></div>
            <div className='h-full'>
                <ul className='flex flex-col justify-center relative -top-24 h-full'>
                    <Link to="/about"><li className='text-xl my-4 flex items-center' onClick={()=>(setNavbar(false))}><IoPersonOutline size={24} className='mr-4'/>About</li></Link>
                    <Link to="/projects"><li className='text-xl my-4 flex items-center' onClick={()=>(setNavbar(false))}><FaLaptopCode size={24} className='mr-4'/>Projects</li></Link>
                    <Link to="contact"><li className='text-xl my-4 flex items-center' onClick={()=>(setNavbar(false))}><MdOutlineMail size={24} className='mr-4'/>Contact</li></Link>
                </ul>
            </div>
        </div>
    </div>
